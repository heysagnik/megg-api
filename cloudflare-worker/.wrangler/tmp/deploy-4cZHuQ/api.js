...
// src/routes/auth.js
var auth = new Hono2();
var SESSION_CACHE_TTL = 3600;
var getSessionUser = /* @__PURE__ */ __name(async (sql, sessionToken, env2) => {
  ...
  try {
    ...
    const ttl = Math.min(SESSION_CACHE_TTL, Math.floor((new Date(session.expires_at) - Date.now()) / 1e3));
    if (ttl > 0 && env2?.CACHE) {
      await env2.CACHE.put(cacheKey, JSON.stringify({
        user,
...
auth.get("/profile", async (c) => {
  try {
    ...
    const [profile3] = await sql`SELECT * FROM users WHERE id = ${user.id}`;
    if (!profile3) {
      await sql`
    ...
    c.executionCtx.waitUntil(
      c.env.CACHE?.put(profileCacheKey, JSON.stringify(profile3), { expirationTtl: 1800 })
    );
...
var wishlist = new Hono2();
var UUID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
var getSessionUser2 = /* @__PURE__ */ __name(async (sql, sessionToken) => {
  if (!sessionToken || sessionToken.length < 10) return null;
  try {
    const [session] = await sql`
    ...
    console.error("Session verification error:", error3.message);
    return null;
  }
}, "getSessionUser");
var authMiddleware = /* @__PURE__ */ __name(async (c, next) => {
  ...
  const token = authHeader.substring(7);
  const sql = cs(c.env.DATABASE_URL);
  const user = await getSessionUser2(sql, token);
  if (!user) {
...
wishlist.get("/", async (c) => {
  try {
    ...
    console.error("Wishlist fetch error:", error3.message);
...
wishlist.post("/:productId", async (c) => {
  try {
    ...
    console.error("Wishlist add error:", error3.message);
...
wishlist.delete("/:productId", async (c) => {
  try {
    ...
    console.error("Wishlist delete error:", error3.message);
...
wishlist.get("/check/:productId", async (c) => {
  try {
    ...
    console.error("Wishlist check error:", error3.message);
...
var MAX_WIDTH = 1200;
var CACHE_TTL = 86400 * 30;
var SUPPORTED_FORMATS = /* @__PURE__ */ new Set(["webp", "jpeg", "png"]);
...
async function handleImageOptimization(request, env2) {
  const url = new URL(request.url);
  ...
  try {
    const cached = await env2.CACHE.get(cacheKey, "arrayBuffer");
    if (cached) {
  ...
  try {
    ...
    env2.CACHE.put(cacheKey, outputBytes.buffer, { expirationTtl: CACHE_TTL }).catch(() => {
    });
    return createImageResponse(outputBytes, format, false);
  } catch (error3) {
    console.error("Image optimization failed:", error3.message);
    return fetch(imageUrl);
  }
}
__name(handleImageOptimization, "handleImageOptimization");
...
var getDB = /* @__PURE__ */ __name((env2) => cs(env2.DATABASE_URL), "getDB");
var validatePagination = /* @__PURE__ */ __name((pageStr, limitStr) => {
  const page = Math.max(1, parseInt(pageStr) || 1);
  const limit = Math.min(100, Math.max(1, parseInt(limitStr) || 20));
  const offset2 = (page - 1) * limit;
  return { page, limit, offset: offset2 };
}, "validatePagination");
...
app.get("/api/health", (c) => c.json({ status: "ok", service: "workers", timestamp: (/* @__PURE__ */ new Date()).toISOString() }));
app.get("/api/products", async (c) => {
  try {
    const { category, subcategory, sort = "popularity" } = c.req.query();
    const { page, limit, offset: offset2 } = validatePagination(c.req.query("page"), c.req.query("limit"));
    ...
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(result), { expirationTtl: CACHE_TTL2.PRODUCT_LIST })
    );
...
app.get("/api/products/brand/:brand", async (c) => {
  try {
    ...
    const { page, limit, offset: offset2 } = validatePagination(c.req.query("page"), c.req.query("limit"));
    const colorFilters = color ? color.split(",").map((c2) => c2.trim()).filter(Boolean) : [];
    ...
    if (colorFilters.length > 0) {
      ...
      params.push(...colorFilters.map((c2) => `%${c2}%`));
      conditions.push(`(${colorConditions.join(" OR ")})`);
    }
    const limitIdx = paramIdx++;
    const offsetIdx = paramIdx++;
    params.push(limit, offset2);
    const orderBy = sort === "price_asc" ? "price ASC" : sort === "price_desc" ? "price DESC" : sort === "newest" ? "created_at DESC" : "popularity DESC";
...
app.get("/api/products/brands", async (c) => {
  try {
    ...
    `;
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(brands), { expirationTtl: CACHE_TTL2.CATEGORIES })
    );
...
app.get("/api/products/:id", async (c) => {
  try {
    ...
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(result), { expirationTtl: CACHE_TTL2.PRODUCT_DETAIL })
    );
    return c.json(result);
  } catch (error3) {
...
app.get("/api/products/:id/recommendations", async (c) => {
  try {
    ...
    if (!product) return c.json({ error: "Product not found" }, 404);
    const recommendations = product.subcategory ? await sql`
    ...
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(result), { expirationTtl: CACHE_TTL2.RECOMMENDATIONS })
    );
    return c.json(result);
  } catch (error3) {
...
app.get("/api/products/:id/variants", async (c) => {
  try {
    ...
    if (!product) return c.json({ error: "Product not found" }, 404);
    const variants = product.subcategory ? await sql`
    ...
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(result), { expirationTtl: CACHE_TTL2.VARIANTS })
    );
    return c.json(result);
  } catch (error3) {
...
app.get("/api/categories", async (c) => {
  try {
    ...
    `;
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(categories), { expirationTtl: CACHE_TTL2.CATEGORIES })
    );
...
app.get("/api/subcategories/:category", async (c) => {
  try {
    ...
    `;
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(results), { expirationTtl: CACHE_TTL2.SUBCATEGORIES })
    );
...
app.get("/api/reels", async (c) => {
  try {
    ...
    if (cached) return c.json(cached);
    const sql = getDB(c.env);
    const reels = category ? await sql`
        ...
        `;
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(reels), { expirationTtl: CACHE_TTL2.REELS })
    );
    return c.json(reels);
  } catch (error3) {
...
app.get("/api/reels/:id", async (c) => {
  try {
    ...
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(result), { expirationTtl: CACHE_TTL2.REEL_DETAIL })
    );
    return c.json(result);
  } catch (error3) {
...
app.get("/api/color-combos", async (c) => {
  try {
    ...
    if (cached) return c.json(cached);
    const sql = getDB(c.env);
    let combos = group_type ? await sql`
        ...
        `;
    if (combos) {
      if (color_a) combos = combos.filter((c2) => c2.color_a === color_a);
      if (color_b) combos = combos.filter((c2) => c2.color_b === color_b);
    }
    const meta = {
      colors: {
        a: [...new Set(combos?.map((c2) => c2.color_a).filter(Boolean))].sort(),
        b: [...new Set(combos?.map((c2) => c2.color_b).filter(Boolean))].sort(),
        c: [...new Set(combos?.map((c2) => c2.color_c).filter(Boolean))].sort()
      }
    };
    const result = { combos: combos || [], meta };
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(result), { expirationTtl: CACHE_TTL2.COLOR_COMBOS })
    );
    return c.json(result);
  } catch (error3) {
...
app.get("/api/color-combos/:id", async (c) => {
  try {
    ...
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(result), { expirationTtl: CACHE_TTL2.COMBO_DETAIL })
    );
    return c.json(result);
  } catch (error3) {
...
app.get("/api/offers", async (c) => {
  try {
    ...
    `;
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(offers), { expirationTtl: CACHE_TTL2.OFFERS })
    );
...
app.get("/api/trending", async (c) => {
  try {
    ...
    `;
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(products), { expirationTtl: CACHE_TTL2.TRENDING })
    );
...
app.get("/api/banners", async (c) => {
  try {
    ...
    if (cached) return c.json(cached);
    const sql = getDB(c.env);
    const banners = category ? await sql`
        ...
        `;
    c.executionCtx.waitUntil(
      c.env.CACHE.put(cacheKey, JSON.stringify(banners), { expirationTtl: CACHE_TTL2.BANNERS })
    );
...
app.all("/api/admin/*", async (c) => {
  try {
    ...
    return fetch(vercelUrl, {
      method: c.req.method,
      headers: c.req.raw.headers,
      body: c.req.method !== "GET" ? await c.req.text() : void 0
    });
  } catch (error3) {
...
app.all("/api/outfits", async (c) => {
  try {
    ...
    if (c.req.method === "GET" && response.ok) {
      c.executionCtx.waitUntil(
        c.env.CACHE.put(cacheKey, JSON.stringify(data), { expirationTtl: CACHE_TTL2.REELS })
      );
    }
...
app.get("/api/search/filters", async (c) => {
  try {
    ...
    if (response.ok) {
      c.executionCtx.waitUntil(
        c.env.CACHE.put(cacheKey, JSON.stringify(data), { expirationTtl: CACHE_TTL2.SEARCH })
      );
    }
    return c.json(data, response.status);
  } catch (error3) {
