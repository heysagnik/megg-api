...
import { deleteOfferBanner } from './upload.service.js';
...
export const listOffers = async ({ page, limit }) => {
  ...
  const cacheKey = `offers:${p}:${l}`;

  return getCached(cacheKey, CACHE_TTL.OFFERS, async () => {
    const offset = (p - 1) * l;
...
export const createOffer = async (offerData) => {
  ...
  invalidateCacheByPrefix('offers:').catch(() => {});
...
export const updateOffer = async (id, updates) => {
  ...
  if (updates.banner_image && existingOffer.banner_image !== updates.banner_image) {
    await deleteOfferBanner(existingOffer.banner_image).catch(() => {});
  }
  ...
  Promise.all([
    invalidateCacheByPrefix(`offer:${id}`),
